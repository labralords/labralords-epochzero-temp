...
const scrape = async () => {
  ...
  for (const repository of repositories) {
    try {
      console.log(`Fetching collections for ${repository.name}`);
      const cursor1 = await cursors.getCursor(repository.name, 'collections');
      console.log(`Cursor ref for ${repository.name} is`, cursor1?.ref);
      await repository.fetchCollections({ ref: cursor1 ? cursor1?.ref : '', name: 'collections' });

      console.log(`Fetching collection updates for ${repository.name}`);
      const cursor2 = await cursors.getCursor(repository.name, 'collection-updates');
      ...
      console.log(`Finished fetching collections for ${repository.name}`);
    } catch (error) {
      console.error(error);
    }
  }
